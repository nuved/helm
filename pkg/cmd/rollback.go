...
func newRollbackCmd(cfg *action.Configuration, out io.Writer) *cobra.Command {
	...
	f.BoolVar(&client.RecoverPending, "recover-pending", false, "if the release is stuck in a pending state (e.g. Helm was killed mid-operation), mark it failed and recover. Only use when no other Helm operation is running against this release")
